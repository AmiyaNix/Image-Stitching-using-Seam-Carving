...
def resize_to_same_height(img1, img2):
    """Resize both images to the same height while maintaining aspect ratio."""
    h1, w1, _ = img1.shape
    h2, w2, _ = img2.shape
...
def find_overlap_width(img1, img2, overlap_ratio=0.2):
    """Finds the width of the overlapping region."""
...
def compute_energy(image):
    """Computes an energy map using the Sobel filter."""
...
def blend_overlap(region1, region2):
    """Applies linear feathering for smooth blending."""
    h, w, _ = region1.shape
